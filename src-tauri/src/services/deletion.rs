...
impl DeletionService {
    ...
    /// Delete multiple files to trash (two-phase: verify then batch-trash)
    #[cfg(test)]
...
mod tests {
    ...
    fn test_verify_batch_mixed() {
        ...
        let good_path = dir.path().join("good.txt");
        let mut f = File::create(&good_path).unwrap();
        f.write_all(b"good").unwrap();
        ...
        assert!(result.failed[0].error.as_ref().unwrap().contains("not found") ||
                result.failed[0].error.as_ref().unwrap().contains("Not found"));
        assert_eq!(result.failed[1].error.as_deref(), Some("File changed since scan"));
    }
