...
/** Shorthand: makeGroup with explicit hash and paths */
function mg(id: number, hash: string, fileSize: number, paths: string[]) {
  return makeGroup(id, fileSize, paths.length, { hash, paths });
}

describe('buildDeletionRequests', () => {
  it('maps pending files to requests with hash and size from groups', () => {
    const result = makeResult([mg(1, 'abc', 1024, ['/a.txt', '/b.txt'])]);
...
describe('buildKeptPathsAndGroupIds', () => {
  it('maps deleted files to kept paths and group IDs', () => {
    const result = makeResult([mg(1, 'abc', 1024, ['/a.txt', '/b.txt', '/c.txt'])]);
...
describe('updateResultsAfterDeletion', () => {
  ...
  it('drops groups with 1 or fewer files remaining', () => {
    const result = makeResult([
      mg(1, 'abc', 1024, ['/a.txt', '/b.txt']),
      mg(2, 'def', 2048, ['/d.txt', '/e.txt', '/f.txt']),
    ]);
...
describe('isDeletingAllInGroup', () => {
  ...
  it('returns true when any one group has all files selected', () => {
    const result = makeResult([
      mg(1, 'abc', 1024, ['/a.txt', '/b.txt', '/c.txt']),
      mg(2, 'def', 2048, ['/d.txt', '/e.txt']),
    ]);
